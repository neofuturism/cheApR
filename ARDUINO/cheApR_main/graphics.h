...
//  zoomB = std::min((float)lcdB.width() / 128, (float)lcdB.height() / 96);
  lcd.setPivot(lcd.width() >> 1, lcd.height() >> 1); lcdB.setPivot(lcdB.width() >> 1, lcdB.height() >> 1);
  lcd.fillScreen(0xFFFFFFU); lcdB.fillScreen(0xFFFFFFU);
...
//  sprite[count].pushRotateZoom(&lcdB, lcdB.width() >> 1, lcdB.height() >> 1, 0, zoomB, zoomB);
