...
{
public:
  bool setup(LovyanGFX* lcd0, LovyanGFX* lcd1)
  {
    _lcd[0] = lcd0;
    _lcd[1] = lcd1;
    Serial.println("TCPReceiver setup.");

    _lcd_width = lcd0->width();
    _lcd_height = lcd0->height();
    _bytesize = lcd0->getColorConverter()->bytes;
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        _dmabufs[i][j] = (uint8_t*)heap_caps_malloc(_lcd_width * 48 * _bytesize, MALLOC_CAP_DMA);
        Serial.printf("buf:%08x \n", (uint32_t)_dmabufs[i][j]);
      }
      _dmabuf[i] = _dmabufs[i][0];
      _lcd[i]->startWrite();
      _lcd[i]->initDMA();
      delay(10);
    }
...
private:
  ...
  static uint32_t jpgWrite16(TJpgD *jdec, void *bitmap, TJpgD::JRECT *rect) {
    ...
    uint16_t *dst = (uint16_t *)me->_dmabuf[0];
  ...
  static uint32_t jpgWriteRow(TJpgD *jdec, uint32_t y, uint32_t h) {
    ...
    if (y == 0) {
      me->_lcd[0]->setAddrWindow(me->_jpg_x, me->_jpg_y, me->_out_width, me->_out_height);
      me->_lcd[1]->setAddrWindow(me->_jpg_x, me->_jpg_y, me->_out_width, me->_out_height);
    }
    ...
    me->_lcd[ flip]->pushPixelsDMA(me->_dmabuf[ flip], me->_out_width * h);
    me->_lcd[!flip]->pushPixelsDMA(me->_dmabuf[!flip], me->_out_width * h);

    flip = !flip;
    me->_dmabuf[0] = me->_dmabufs[0][flip];
    me->_dmabuf[1] = me->_dmabufs[1][flip];
    return 1;
  }
  ...
  bool drawJpg() {
    ...
    _out_width = std::min<int32_t>(_jdec.width>>1, _lcd_width);
    _jpg_x = (_lcd_width - (_jdec.width>>1)) >> 1;
    if (0 > _jpg_x) {
      _off_x = - _jpg_x;
      _jpg_x = 0;
    } else {
